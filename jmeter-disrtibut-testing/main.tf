...
resource "aws_instance" "jmeter_client_instance" {
  ...
  provisioner "file" {
    source      = "./test.jmx"
    destination = "/home/ubuntu/test.jmx"
...
locals {
  ...
  jmeter_server_ips = join(",", [for server in local.jmeter_servers : server.public_ip])
...
resource "null_resource" "start_jmeter_test" {
  ...
  provisioner "remote-exec" {
    inline = [
      "sleep 20s",
      "JVM_ARGS='${var.jvm_args}' && export JVM_ARGS",
      "screen -dmS jmeter_session bash -c '/home/ubuntu/jmeter/bin/jmeter -n -t test.jmx -R ${local.jmeter_server_ips} -Dserver.rmi.ssl.keystore.file=/home/ubuntu/jmeter/bin/rmi_keystore.jks -l data/log.jtl -j data/test.log -e -o data/out > /home/ubuntu/jmeter-clien.log 2>&1'", 
      "sleep 5s"
    ]
